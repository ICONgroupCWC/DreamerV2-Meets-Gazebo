...
class ObsEncoder(nn.Module):
    def __init__(self, input_shape, embedding_size, info):
        ...
        self.convolutions = nn.Sequential(
            nn.Conv2d(input_shape[0], d, k[0]),   #nn.Conv2d(input_shape[0], d, k), edit josh
            activation(),
            nn.Conv2d(d, 2*d, k[1]),              #nn.Conv2d(d, 2*d, k), edit josh
            activation(),
            nn.Conv2d(2*d, 4*d, k[2]),            #nn.Conv2d(2*d, 4*d, k), edit josh
            activation(),
        )
