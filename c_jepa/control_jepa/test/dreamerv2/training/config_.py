...
class MinAtarConfig:
    ...
    horizon: int = 10
    lr: Dict = field(
        default_factory=lambda: {"model": 2e-4, "actor": 4e-5, "critic": 1e-4}
    )
    loss_scale: Dict = field(
        default_factory=lambda: {"kl": 0.1, "reward": 1.0, "discount": 5.0}
    )
    kl: Dict = field(
        default_factory=lambda: {
            "use_kl_balance": True,
...
@dataclass
class RacingCarConfig:
    """default HPs that are known to work for MinAtar envs"""

    # env desc
    env: str = "CarRacing-v2"
    obs_shape: Tuple = (1, 84, 84)
    action_size: int = 5
    pixel: bool = True
    action_repeat: int = 1
    ...
    train_every: int = 20  # reduce this to potentially improve sample requirements
    collect_intervals: int = 5
    batch_size: int = 32
    seq_len: int = 50
    eval_episode: int = 4
    eval_render: bool = True
    save_every: int = int(2e4)
    seed_steps: int = 3000  # 4000
    model_dir: int = "results"
    gif_dir: int = "results"
    ...
    loss_scale: Dict = field(
        default_factory=lambda: {"kl": 0.5, "reward": 1.0, "discount": 5.0}
    )
    kl: Dict = field(
        default_factory=lambda: {
            "use_kl_balance": True,
...
class MiniGridConfig:
    ...
    loss_scale: Dict = field(
        default_factory=lambda: {"kl": 1, "reward": 1.0, "discount": 10.0}
    )
    kl: Dict = field(
        default_factory=lambda: {
            "use_kl_balance": True,
