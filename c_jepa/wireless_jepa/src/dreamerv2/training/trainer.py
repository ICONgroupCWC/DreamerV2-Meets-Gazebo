...
class Trainer(object):
    ...
    def collect_seed_episodes(self, env):
        s, _ = env.reset()  # unpack obs and info
        done = False
        for i in range(self.seed_steps):#
            print("seeds steps : " ,i)
            a = env.action_space.sample()
    ...
    def train_batch(self, train_metrics):
        ...
        for i in range(self.collect_intervals):
            ...
            # print(actions.shape)
            # print(rewards.shape)
            # print(torch.tensor(1-terms, dtype=torch.float32))
            # print("**********************")
    ...
    def actorcritc_loss(self, posterior):
        ...
        with FreezeParameters(self.world_list):
            imag_rssm_states, imag_log_prob, policy_entropy,_ = self.RSSM.rollout_imagination(self.horizon, self.ActionModel, batched_posterior)
    ...
    def save_model(self, iter):
        ...
        save_every = 20000
        if(iter>save_every):
            rm_model_dir = os.path.join(model_dir, 'models_%d.pth' % (iter-save_every))
            os.system(f"rm -rf {rm_model_dir}")
